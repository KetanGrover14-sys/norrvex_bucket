...
function key() {
  ...
  if (!secret || secret.length < 32 || secret.startsWith('replace-with-')) {
    throw Object.assign(new Error('Bucket login is not configured. Set SESSION_SECRET to a random value of at least 32 characters in the server environment, then restart or redeploy.'), { status: 503, code: 'SESSION_CONFIG' });
  }
  return createHash('sha256').update(secret).digest();
}

export function validateSessionConfiguration() { key(); }

import { requestSession, sealSession, sessionCookie, validateSessionConfiguration } from './session.js';
...
async function upstream(route, { method = 'GET', token, body, contentType } = {}) {
  let base;
  try { base = new URL(process.env.WECAPURRED_URL?.trim() || 'https://norrvex-partners.procurre.in'); }
  catch { throw fail('WECAPURRED_URL is invalid. Set it to the HTTPS origin of the partner website.', 503); }
  ...
  let response;
  try {
    response = await fetch(new URL(route, base), { method, headers, body, redirect: 'manual', cache: 'no-store', signal: AbortSignal.timeout(20000) });
  } catch (error) {
    const timeout = ['TimeoutError', 'AbortError'].includes(error.name);
    console.error('[Bucket backend connection]', { route, host: base.host, code: error.cause?.code || error.name });
    throw fail(timeout ? 'The partner backend timed out. Please retry shortly.' : 'Cannot reach the partner backend. Check WECAPURRED_URL and the Bucket server network connection.', timeout ? 504 : 502);
  }
  if (response.status >= 300 && response.status < 400) {
    throw fail('The partner API redirected this request. Set WECAPURRED_URL to the final partner website origin.', 503);
  }
  let data;
  try { data = await response.json(); }
  catch {
    throw fail(`The partner backend returned HTTP ${response.status} without a JSON response. Check the partner deployment and its server logs.`, 502);
  }
